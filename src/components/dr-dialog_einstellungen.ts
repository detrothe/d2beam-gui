...
export class drDialogEinstellungen extends LitElement {
      ...
      return html` <dialog id="dialog_einstellungen">
         ...
         <table>
            ...
            <tbody>
               <tr>
                  <td>dx:</td>
                  <td><input type="number" id="id_dx" name="dx" pattern="[0-9.,eE+-]*" value="0.5" /></td>
                  <td>[m]&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
                  <td>x:</td>
                  <td><input type="number" id="id_rasteroffset_x" name="offset_x" pattern="[0-9.,eE+-]*" value="0.0" /></td>
                  <td>[m]</td>
               </tr>
               <tr>
                  <td>dz:</td>
                  <td><input type="number" id="id_dz" name="dz" pattern="[0-9.,eE+-]*" value="0.5" /></td>
                  <td>[m]&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
                  <td>z:</td>
                  <td><input type="number" id="id_rasteroffset_z" name="offset_x" pattern="[0-9.,eE+-]*" value="0.0" /></td>
                  <td>[m]</td>
               </tr>
            </tbody>
         </table>
   ...
   set_rasterOffset_x(fw: number) {
      let el = this.shadowRoot?.getElementById('id_rasteroffset_x') as HTMLInputElement;
      el.value = String(fw);
   }

   set_rasterOffset_z(fw: number) {
      let el = this.shadowRoot?.getElementById('id_rasteroffset_z') as HTMLInputElement;
      el.value = String(fw);
   }

   get_rasterOffset_x() {
      const shadow = this.shadowRoot;
      let wert = +(shadow?.getElementById('id_rasteroffset_x') as HTMLInputElement).value.replace(/,/g, '.');
      return wert;
   }

   get_rasterOffset_z() {
      const shadow = this.shadowRoot;
      let wert = +(shadow?.getElementById('id_rasteroffset_z') as HTMLInputElement).value.replace(/,/g, '.');
      return wert;
   }
   ...
   _saveLocalStorage() {
      ...
      if (shadow) {
         ...
         wert = Math.abs(+(shadow?.getElementById('id_rasteroffset_x') as HTMLInputElement).value.replace(/,/g, '.'));
         window.localStorage.setItem('cad_id_rasteroffset_x', String(wert));

         wert = Math.abs(+(shadow?.getElementById('id_rasteroffset_z') as HTMLInputElement).value.replace(/,/g, '.'));
         window.localStorage.setItem('cad_id_rasteroffset_z', String(wert));
      }
   }
}

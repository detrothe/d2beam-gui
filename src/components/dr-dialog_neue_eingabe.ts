...
export class drDialogNeueEingabe extends LitElement {
  ...
  render() {
    return html` <dialog id="dialog_neue_eingabe">
      ...
      <p>
        Die Tabellengrößen können später jederzeit mit den Buttons, die sich oberhalb der Tabellen befinden, geändert werden.<br />
      ...
      <p>Die Art des Systems kann nachträglich nicht mehr geändert werden !</p>
  ...
  _dialog_ok() {
    ...
    if (shadow) {
      (shadow.getElementById("dialog_neue_eingabe") as HTMLDialogElement).close("ok");
    }
  }
  ...
  _dialog_abbruch() {
    ...
    const shadow = this.shadowRoot;
    if (shadow) (shadow.getElementById("dialog_neue_eingabe") as HTMLDialogElement).close("cancel");
  }
}
